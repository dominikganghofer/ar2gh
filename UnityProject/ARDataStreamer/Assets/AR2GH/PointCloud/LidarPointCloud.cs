...
{
    public event Action<CloudPoint[]> CloudUpdateEvent;
    ...
    [SerializeField]
    private DebugViews _debugViews = default;
    ...
    private void UpdateEnvironmentDepthImage()
    {
        ...
        using (image)
        {
            ...
            {
                _depthTextureFloat = new Texture2D(image.width, image.height, image.format.AsTextureFormat(), false);
    ...
    private void ReprojectPointCloud()
    {
        ...
        {
            _cloud = new CloudPoint[widthDepth * heightDepth];
    ...
    private static CloudPoint[] GenerateDebugCloud()
    {
        const int count = 10000;
        var cloud = new CloudPoint[count];
        for (var i = 0; i < count; i++)
        {
            var t = 1f * i / count;
            var randomPosition = 10 * Random.insideUnitSphere;
            cloud[i] = new CloudPoint()
            {
                SnappedPositionWorld = new Vector3Int(Mathf.RoundToInt(randomPosition.x), Mathf.RoundToInt(randomPosition.y),
